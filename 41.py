            ...
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7    #判斷左下與右下斜角相連
                    if self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]:
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7    #判斷右上向左下斜角相連
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for y in range(8, -1, -1):    #從最下方第9列開始向上到第1列檢查 y+1=第幾列
            for x in range(8):    #從最左方第1行開始向右到第8行檢查 x+1=第幾行
                if self.neko[y][x] > 0:    #如果這格有方塊
                    if self.neko[y+1][x] == 0:    #檢查下方是否有方塊，若是沒有此格的方塊就移至下方
                        self.neko[y+1][x] = self.neko[y][x]    
                        self.neko[y][x] = 0

        drop = True    #檢查是否全部方塊都已落下，如果已完成Drop == True結束，若是還沒完成Drop == False繼續
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        over = False
        for x in range(8):    #檢查最上方第1列是否有方塊，x由左至右
            if self.neko[0][x] > 0:
                over = True 
        return over
