...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for x in range(1, 7):
            for y in range(10):
                if self.check[y][x] > 0:
                    if self.check[y][x - 1] == self.check[y][x] and self.check[y][x + 1]:
                        self.neko[y][x - 1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x + 1] = 7
        ...
        for y in range(1, 9):
            for x in range(1,7):
                if self.check[y][x] > 0:
                    if self.check[y - 1][x + 1] == self.check[y][x] and self.check[y + 1][x - 1]:
                        self.neko[y - 1][x + 1] = 7
                        self.neko[y][x] = 7
                        self.neko[y + 1][x - 1] = 7
#========================================================= Question 2 =========================================================
            ...
            for x in range(8):
                if self.neko[y][x] > 0:
                    if(self.neko[y + 1][x] == 0):
                        ...
                        self.neko[y][x] = 0
                        return False #true or false both run success
    #判斷是否全部掉完 如果沒有 return false
    #掉完 return true
                        
#========================================================= Question 3 =========================================================
    # 題目說明：
    # 1.判斷貓咪方塊是否碰到最上層，是則回傳 True，否則回傳 False。
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        flag = False
