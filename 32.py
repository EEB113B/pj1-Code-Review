...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for y in range(9, 0, -1):
            for x in range(8):
                if self.neko[y][x] == 0:
                    self.neko[y][x] = self.neko[y-1][x]          #從下面判斷，若判斷格為空白，則將判斷格上方的圖片複製到判斷格
                    self.neko[y-1][x] = 0                        #再將判斷個上方設為空白
                if y < 9:
                    if self.neko[y][x] >0 and self.neko[y+1][x] == 0:
                        the_bottom = False                       #若the_bottom為False，表示判斷格漂浮在空中，還未到最低地方，則不會進入step 3
        return the_bottom       
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        y = 0
        for x in range(8):
            if self.neko[y][x] != 0:   #判斷第一列是否有貓咪圖片，有表示已經碰到最上層，即game over
                game_over = True
        return game_over
