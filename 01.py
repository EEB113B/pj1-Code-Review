        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    ...
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1]:   #左右相等
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7
                    
                    if  self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1]:  #左下到右上斜對角
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7
                    
                    if  self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1]:  #左上到右下斜對角
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        stop = True
        for y in range(8, -1, -1):                                           # y 8跑到0
            for x in range(8):                                               # x 0到7
                if self.neko[y][x] != 0 and self.neko[y+1][x] == 0:          # 第N行不等於0 且 N+1行等於0 直到N+1行不等於0跳出迴圈 回傳True
                    self.neko[y+1][x] = self.neko[y][x]                      # 第N行複製到N+1行
                    self.neko[y][x] = 0                                      # 清空第N行
                    stop = False
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        for x in range(8):            #檢查最上面那一行
            if self.neko[0][x] > 0:   #如果有圖片(大於0) return True  
                return True
        return False
#==============================================================================================================================
newgame = CatGame()
