...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0, 10): 
            for x in range(8):
                if self.check[y][x] > 0: #若格子中有圖案
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]: #若左邊的圖案等於右邊與中間
                        self.neko[y][x-1] = 7 #左邊指派肉球圖
                        self.neko[y][x] = 7 #中間指派肉球圖
                        self.neko[y][x+1] = 7 #右邊指派肉球圖
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0: #若格子中有圖案
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:#若左上圖案等於右下與中間
                        self.neko[y+1][x-1] = 7 #左上指派肉球
                        self.neko[y][x] = 7     #中間指派肉球
                        self.neko[y-1][x+1] = 7#右下指派肉球
                    if self.check[y+1][x+1] == self.check[y][x] and self.check[y-1][x-1] == self.check[y][x]:#若右上圖案等於左下與中間
                        self.neko[y-1][x-1] = 7#左下指派肉球
                        self.neko[y][x] = 7     #中間指派肉球
                        self.neko[y+1][x+1] = 7#右上指派肉球
#========================================================= Question 2 =========================================================
    # 題目說明(此題有兩項要求)：
    ...
    def drop_neko(self):
        ...
        for y in range(9):
            for x in range(8):
                if self.neko[y+1][x] == 0:#若下一格沒有方塊
                    self.neko[y+1][x] = self.neko[y][x]#上一格圖案往下一格指派
                    self.neko[y][x] = 0  #上一格圖案消除
                   
                elif self.neko[y+1][x] != 0:#若下一格有方塊
                    self.neko[y][x] = self.neko[y][x]#停留在原格子
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):
            if self.neko[0][x]>0:#若第一列(最上方)大於0
                return True
            else:
                return False
