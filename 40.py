...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x] == self.check[y][x] and self.check[y+1][x] == self.check[y][x]:       # 在相同鉛直方向下，如果上面等於中間，且下面等於中間
                        self.neko[y-1][x] = 7                                                                   # 將下面的格子變成肉球
                        self.neko[y][x] = 7                                                                     # 將中間的格子變成肉球
                        self.neko[y+1][x] = 7                                                                   # 將上面的格子變成肉球
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:       # 在相同水平高度下，如果左邊等於中間，且右邊等於中間
                        self.neko[y][x-1] = 7                                                                   # 將左邊的格子變成肉球
                        self.neko[y][x] = 7                                                                     # 將中間的格子變成肉球
                        self.neko[y][x+1] = 7                                                                   # 將右邊的格子變成肉球
                                                                                                                # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:   # 如果左上到右下皆相等
                        self.neko[y-1][x-1] = 7                                                                 # 將左上的格子變成肉球                      
                        self.neko[y][x] = 7                                                                     # 將中間的格子變成肉球
                        self.neko[y+1][x+1] = 7                                                                 # 將右下的格子變成肉球
                                                                                                                # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去                        
                    
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:   # 如果左下到右上相等
                        self.neko[y+1][x-1] = 7                                                                 # 將左下的格子變成肉球
                        self.neko[y][x] = 7                                                                     # 將中間的格子變成肉球
                        self.neko[y-1][x+1] = 7                                                                 # 將右上的格子變成肉球
                                                                                                                # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):                                          # X 的座標有 0~7 總共八個
            for i in range(10):                                     # 為了讓下面的程式一直檢查，先設立一個 for 迴圈來幫助檢查，從 0~9 跑，共十次
                for y in range(0,9):                                # Y 的座標有 0~8 總共九個
                    if self.neko[y+1][x] == 0:                      # 相較於 [y][x] ，如果它下面的 [y+1][x] 沒有東西
                        self.neko[y+1][x] = self.neko[y][x]         # 則複製 [y][x] 的貓咪到下面的 [y+1][x] 去
                        self.neko[y][x] = 0                         # 並且刪除複製前 [y][x] 的貓咪                       
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        GameOver = False                    # 預設 GameOver 變數為 False
        for x in range(8):                  # X 的座標有 0~7 總共八個
            y = 0                           # 預設直接判斷最上層
            if self.neko[y][x] != 0:        # 如果最上層有東西，代表已經碰到頂端了
                GameOver = True             # 將 GameOver 變數設為 True
        return GameOver                     # 回傳 GameOver 變數
