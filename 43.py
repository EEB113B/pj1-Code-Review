...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(10): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]: #判斷左右
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]: #判斷右上左下斜角
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x+1] == self.check[y][x] and self.check[y-1][x-1] == self.check[y][x]: #判斷左上右下斜角
                        self.neko[y+1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                        self.neko[y][x] = 7     # 同上
                        self.neko[y-1][x-1] = 7 # 同上
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
            ...
            for y in range(8, -1, -1):                                             #從下往上確認是否有空格
                for x in range(8): 
                    if self.neko[y][x] != 0 and self.neko[y+1][x] == 0:            #如果此格不等於0(也就是有貓咪)且落下的下一格為空格(為0)
                        self.neko[y+1][x] = self.neko[y][x]                        #則此格複製到下一格
                        self.neko[y][x] = 0                                        #並將此格改為空格(等於0)
                        re = False                                                 #回傳False，直到沒有空格，才回傳True
            return re
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):
            if self.neko[0][x] > 0:                                                #當矩陣第一行(第0排)> 0時，則回傳True=執行
                return True
        return False                                                               # <= 0時，回傳False
