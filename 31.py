    ...
    def check_neko(self):
       ...
       # 2.判斷左右是否連線
        ...
        for y in range(1, 9): 
            for x in range(1, 7):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y][x] == self.check[y+1][x+1]:
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7
        for y in range(1, 9): 
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        for y in range(8, -1, -1):
            for x in range(8):                                      
                if self.neko[y+1][x] == 0:                               #判斷每格的面是否為0，如果是就往下掉一格
                    self.neko[y+1][x] = self.neko[y][x]
                    self.neko[y][x] = 0                     
        count = 0                                                   
        for y in range(9):                                              
            for x in range(8):
                if self.neko[y+1][x] == 0 and self.neko[y][x] != 0:      #確認是否所有格子都不能再往掉
                    return False
                count += 1
                if count == 72:
                    count = 0
