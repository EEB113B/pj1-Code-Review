...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]: #右上左下連線
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去                                      
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: #右下左上連線
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        #drop = False      
        for x in range(8):  #8列
            for i in range(10):    #檢查還沒掉落的方塊
                for y in range(0,9): #9行          
                    #if self.neko[y][x]>0:   #如果有貓咪方塊
                    if self.neko[y+1][x]==0:    #如果下一格是空格                        
                        self.neko[y+1][x]=self.neko[y][x]  #複製到下一格                
                        self.neko[y][x]=0   #刪除原本方塊                                                              
        return True  #全部方塊落下完畢 
                    
        """                             
                    if x==7 and i==9 and y==0:#移動結束
                        for x in range(8):
                            for y in range(8,-1,-1):    #從下往上看
                                if self.neko[y-1][x]!=0 and self.neko[y+1][x]==0:   #格子下面有東西，上面沒東西
                                    drop = True     #全部方塊落下完畢
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        for x in range(8):
            y=0     #設定在最上層
            if self.neko[y][x]!=0:  #假如最上層有貓咪方塊
                over=True   #over變成True
        return over #回傳 
