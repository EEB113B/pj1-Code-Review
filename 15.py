...
class CatGame:
    def __init__(self):
        ...
        self.canvas.pack()
        self.background = tk.PhotoImage(file="C:/project1/picture/neko_bg.png")
        self.cursor = tk.PhotoImage(file="C:/project1/picture/neko_cursor.png")
        self.img_neko = [None,
                        tk.PhotoImage(file="C:/project1/picture/neko1.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko2.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko3.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko4.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko5.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko6.png"),
                        tk.PhotoImage(file="C:/project1/picture/neko_niku.png")]
...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(0, 9):
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x] == self.check[y+1][x-1] and self.check[y-1][x+1] == self.check[y][x]: #確認左下及右上方塊是否與中間相同
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7
                        self.neko[y-1][x+1] = 7
                        #將三格方塊變為肉球圖示
        ...
        for y in range(0, 9):
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x] == self.check[y+1][x+1] and self.check[y-1][x-1] == self.check[y][x]: #確認右下及左上方塊是否與中間相同
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7
                        self.neko[y-1][x-1] = 7
                        #將三格方塊變為肉球圖示
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        time = 0 #設定一次數變數
        while time < 9: #
            for y in range(8,-1,-1): #從倒數第二排開始往上確認
                for x in range(0,9):
                    if self.neko[y][x] != 0 and self.neko[y+1][x] == 0: #確認是否貓咪方塊下有空格
                        self.neko[y+1][x] = self.neko[y][x] #如果有則將空格變為上一格之貓咪方塊
                        self.neko[y][x] = 0 #將原先方塊變為空格
            time += 1 #重複9次
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        for x in range(0,9): 
            if self.neko[0][x] != 0: #確認第一排是否有非空格的矩陣，如果有回傳True
                return True
        return False #如果無則回傳False
