...
class LinkTest:
    ...
    def game_main(self):
        ...
        # 點選氣球文字的測試, 呼叫check_neko方法
        if 660 <= self.mouse_x < 840 and 100 <= self.mouse_y and self.mouse_y < 160 and self.mouse_click == 1:
            self.mouse_click = 0
            self.check_neko()
        ...
        if 24 <= self.mouse_x and self.mouse_x < 24 + 72 * 8 and 24 <= self.mouse_y and self.mouse_y <  24 + 72 * 10:
            ...
            if self.mouse_click == 1:
                self.mouse_click = 0
                self.neko[self.cursor_y][self.cursor_x] = random.randint(1, 1)
