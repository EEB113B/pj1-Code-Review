...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0: #如果左邊等於中間而且中間也等於右邊
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0: #如果左邊上面等於中間而且中間也等於右邊下面(左上到右下的斜線)
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0: #如果左邊下面等於中間而且中間也等於右邊上面(右上到左下的斜線)
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7 #此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        for y in range(9, 0, -1): 
            for x in range(0, 8):
                if self.neko[y][x] == 0: #如果此行為零
                    self.neko[y][x] = self.neko[y-1][x]#把上面那行的圖案傳給此行(也就是下降)
                    self.neko[y-1][x] = 0 #將上一行變成零
        for y in range(9, 0, -1): 
            for x in range(0, 8):
                if self.neko[y][x] == 0:#如果此行為零但上面那行不為零，代表還沒降到最下面，所以return False
                    if self.neko[y-1][x] != 0:
                        return False 
        return True #for如果成功跑完，代表所有可以下降的圖案都下降完畢，所以return True
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(0, 8 ):
            if self.neko[0][x] != 0: #如果第一行不為零但第二行為零則可以繼續下降，所以必須第一行及第二行都不為零，代表不能下降而且碰到最上面
                if self.neko[1][x] != 0:
                    return True
        return False
