...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        neko_touch_bottom = True # 將neko_touch_bottom預設成True
        for y in range(9,0,-1): # 從第9排開始往上判斷(224到228是落下一格的程式碼)(229到231是判斷是否都落到底下的程式碼)
            for x in range(8):
                if self.neko[y][x]==0: # 如果格子為0
                    self.neko[y][x]=self.neko[y-1][x] # 抓上面那一格的值
                    self.neko[y-1][x]=0 # 將格子上面那格歸0
                if y < 9: # 判斷0到8排
                    if self.neko[y][x]>0 and self.neko[y+1][x]==0: # 如果有一格自己大於0，他下面那格卻是0
                        neko_touch_bottom = False # 將neko_touch_bottom設成False
        return neko_touch_bottom # 回傳neko_touch_bottom                                                          
#========================================================= Question 3 =========================================================
    # 題目說明：
    # 1.判斷貓咪方塊是否碰到到最上層，是則回傳 True，否則回傳 False。
    def over_neko(self):
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        touch_top = False # 將touch_top預設成False
        for x in range(8):
            if self.neko[0][x]>0:#如果第0排有一格大於0，代表碰到最上層 
                touch_top = True # 將touch_top預設成True
        return touch_top # 回傳touch_top
#==============================================================================================================================
newgame = CatGame()
newgame.run()