...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for y in range(-1,9): 
            for x in range(8):          #掃描每一格
                if self.check[y][x] > 0:          #如果當下那格有東西的話      
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:   #判斷左,右各一格內容是否與現在這格相同
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
        ...
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1,9):    #掃描每一格
            for x in range(1,7):
                if self.check[y][x] > 0:  #如果當下那格有東西的話 
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: #判斷左下,右上內容是否與現在這格相同
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去

                    if self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]: #判斷左上,右下內容是否與現在這格相同
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(9):  #共8攔
            for i in range(10):  #共10行
                for y in range(8,-1,-1): #由下往上掃,一次移動一格
                    if self.neko[y+1][x] == 0: #如果那格的下一格為空格,則將本格複製到下一格
                        self.neko[y+1][x] =  self.neko[y][x]
                        self.neko[y][x] = 0 #複製完後把當下這格刪除 以免複製
        return  True #可判斷所以有格子皆在可移動的最底下
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
        for x in range (8):  #由右到左掃描每一格
            y = 0 #掃描最上面那行
            if self.neko[y][x] != 0: #有任何一格不為空格的話c + 1
                c += 1 
        if c == 0: #只有c為0的時候才回傳 false
            return False
