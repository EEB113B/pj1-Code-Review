...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1,10):                                                             #從上到下取值
            for x in range(1, 9):                                                          #從左到右取值
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:     #判斷左右是否一樣
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7 # 相連消去
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        for y in range(8, -1, -1):                           
            for x in range(8):                                  
                if self.neko[y][x] > 0:                         #出現貓咪的條件，並且y軸下一層是空格
                    if self.neko[y+1][x] == 0:
                        self.neko[y+1][x] = self.neko[y][x]     #值傳給下一層
                        self.neko[y][x] = 0                     #上一層設空白
        
        T = True
        for y in range(8, -1, -1):                 #檢查貓咪有沒有掉到最下層
            for x in range(8):
                if self.neko[y][x] > 0:
                   if self.neko[y+1][x] == 0:
                       T = False
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        for x in range(8):
            if self.neko[0][x] > 0:    #用來判斷最上層有沒有方塊
                return True
        return False
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
        # <<<將你的程式寫在這邊>>>
