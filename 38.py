...
class CatGame:
    ...
    def game_main(self):
        ...
        self.root.after(50, self.game_main) #100ms後在執行game_main
...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        #2.判斷左右是否連線
        for y in range(10):
            for x in range(0+1,8-1):#第0個開始為中心向左右兩側比對，直到第7個為中新比對完為止
                if self.check [y][x] > 0:
        ...
        for y in range(1,9):
            for x in range(1,7):
                if self.check[y][x] >0:
                    if self.check[y-1][x-1] ==  self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: #左上右下型
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                    elif\
                        self.check[y+1][x-1] ==  self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]: #右上左下型
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        for row in range(-2,-11,-1):#從下層row向上跑
            for collumn in range(8):#每層中元素
                #檢查欄位下方是否為空格
                if self.neko[row+1][collumn] == 0:
                    self.neko[row+1][collumn] = self.neko[row][collumn] #向下遞補
                    self.neko[row][collumn] = 0 # 本欄向下移動，並歸0
                #如下方已有喵咪
                else:
                    if self.neko[row][collumn] == 0:
                    #判斷本欄是否為空格
                    #最上一列不須動作
                        if row == -10:
                            continue
                    # 將上欄元素向下遞補
                        self.neko[row][collumn] = self.neko[row-1][collumn]
                        self.neko[row-1][collumn] =0 #遞補後上欄歸0
                    else:
                    #非空格則不動作，進入下一元素
                        continue
        #判斷是否需要再向下降落一行
        for collumn in range(8):
            for row in range(-2, -10, -1):
                if self.neko[row][collumn] !=0: # 找到直行中的喵咪，判斷下層是否為空格
                    if self.neko[row+1][collumn]==0:
                        return False # 有空格->再做一次降落
        #皆為空格 或皆無空格在兩喵咪中間時
        return True #準備進入step3
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        for column in range(8):
            if self.neko[0][column] !=0:#最上層被佔據 則無法再降落
                return True
        return False 
        # hint : 此方法需要回傳值(return)，型態為布林值(bool : True, False)
