...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        for y in range(1,10):                                                                                 #y軸從上到下取值
            for x in range(1, 9):                                                                             #x軸從上左到右取值
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:     #$判斷左右兩邊是否相等
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
        ...
        for y in range(1, 9): 
            for x in range(1, 9):
                if self.check[y][x] > 0:
                    if self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]: #$判斷左下右上連線是否相等
                        self.neko[y+1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x+1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                    if self.check[y+1][x+1] == self.check[y][x] and self.check[y-1][x-1] == self.check[y][x]: #$判斷左上右下連線是否相等
                        self.neko[y+1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x-1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for y in range(8,-1,-1):                            #y軸層底下開始往上一層一層讀值，for迴圈跑完後整體貓咪往下掉一格
            for x in range(8):                              #x軸上一格一格判斷
                if self.neko[y][x] > 0:                     #如果出現貓咪的條件下，且y軸下一層是空格
                    if self.neko[y+1][x] == 0:
                        self.neko[y+1][x] = self.neko[y][x] #把上一層的值給下一層
                        self.neko[y][x] = 0                 #上一層設為空白(表示貓咪往下掉了)
        
        a = True
        for y in range(8,-1,-1):                             #一層一層檢查是否貓咪已經掉到最下層了
            for x in range(8):
                if self.neko[y][x] > 0:
                   if self.neko[y+1][x] == 0:
                       a = False
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):
            if self.neko[0][x] > 0:    #判斷最上層是否存在貓咪方塊
                return True
        return False
