...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        # <<<將你的程式寫在這邊>>>
        ##若X方向三個貓咪或以上一樣且連線，就消除
        for y in range(10): 
            for x in range(1,8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:   ##判斷中間的貓咪是否等於左右貓咪
                        self.neko[y][x-1] = 7                         ##左邊消除
                        self.neko[y][x] = 7                           ##中間消除
                        self.neko[y][x+1] = 7                         ##右邊消除
        # 3.判斷斜角是否連線
        # <<<將你的程式寫在這邊>>>
        ##左上到右下，若三個貓咪或以上一樣且連線，就消除
        for y in range(1,9): 
            for x in range(1,8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]: ##判斷中間的貓咪是否等於左上及右下貓咪
                        self.neko[y-1][x-1] = 7                       ##左上消除
                        self.neko[y][x] = 7                           ##中間消除
                        self.neko[y+1][x+1] = 7                       ##右下消除
        ##右上到左下，若三個貓咪或以上一樣且連線，就消除
        for y in range(1,9): 
            for x in range(1,8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]: ##判斷中間的貓咪是否等於右上及左下貓咪
                        self.neko[y-1][x+1] = 7                       ##右上消除
                        self.neko[y][x] = 7                           ##中間消除
                        self.neko[y+1][x-1] = 7                       ##左下消除
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        staging=True                                          ##將布林值True給staging暫存區存入
        
        for y in range(8,-1,-1):                        ##將程式由下往上掃
            for x in range(8):                          ##將每個x座標掃入
                if self.neko[y+1][x]>0:                 ##若貓咪格子的下方格子有貓咪，則跳過
                    continue      
                else:                                   ##相反，若貓咪格子的下方為空格子，向下執行
                    self.neko[y+1][x]=self.neko[y][x]   ##將貓咪格子中的貓咪傳給下方的空格子
                    self.neko[y][x]=0                   ##將原貓咪格子中的貓咪刪掉
                    

        for y in range(8,-1,-1):
            for x in range(8):   
                if self.neko[y+1][x]==0 and self.neko[y][x]>0:    ##判斷每個貓咪是否已下落到可下落的最下層
                    staging=False                                       ##若是沒有，則將staging中的布林值改成False
       
        return staging                                       ##回傳staging(布林值)
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for x in range(8):
            if self.neko[0][x]>0:      ##判斷程式在掉落且消掉後是否在最上層還遺留有方塊
                return True            ##若有回傳True，結束全部程式
        return False                   ##若無回傳False，方塊繼續掉落
