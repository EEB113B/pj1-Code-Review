...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1, 10): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x+1] == self.check[y][x] and self.check[y][x-1] == self.check[y][x]:#判斷中間貓咪方塊右側與左側是否同色
                        self.neko[y][x+1] = 7#將右側方塊換成肉球圖片
                        self.neko[y][x] = 7#將中間方塊換成肉球圖片
                        self.neko[y][x-1] = 7#將左側方塊換成肉球圖片
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x+1] == self.check[y][x] and self.check[y-1][x-1] == self.check[y][x]:#判斷貓咪方塊右上與左下是否同色
                        self.neko[y+1][x+1] = 7#將右上方塊換成肉球圖片
                        self.neko[y][x] = 7#將中間方塊換成肉球圖片
                        self.neko[y-1][x-1] = 7#將左下方塊換成肉球圖片
                    
                    elif self.check[y+1][x-1] == self.check[y][x] and self.check[y-1][x+1] == self.check[y][x]:#判斷貓咪方塊左上與右下是否同色
                        self.neko[y+1][x-1] = 7#將左上方塊換成肉球圖片
                        self.neko[y][x] = 7#將中間方塊換成肉球圖片
                        self.neko[y-1][x+1] = 7#將右下方塊換成肉球圖片
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        output=True#將原本的回傳值訂為True
        for y in range(9,0,-1):#由底層第9列開始判斷 
            for x in range(8):
                if self.neko[y][x] == 0 :
                    self.neko[y][x] = self.neko[y-1][x]#如果此位置為0，讓上方的方塊取代其值(落下)
                    self.neko[y-1][x] = 0#方塊落下後，原本位置的值變為0
                if y<9:
                    if self.neko[y][x] != 0 and self.neko[y+1][x] == 0:
                        output=False#方塊還沒落完前將回傳值訂為False
        return output#方塊落完後回傳True，進到下一步
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        reach_top = False
        for x in range(8):
            if self.neko[0][x] != 0  :#檢查頂層是否有方塊
                reach_top = True#如果頂層有方塊的話，Game Over
        return reach_top
