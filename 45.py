...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:#檢查與左右相鄰的格子圖案是否相同
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7#是的話，將全部轉換為肉球圖案
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y-1][x-1] == self.check[y][x] and self.check[y+1][x+1] == self.check[y][x]:#檢查與斜角的格子圖案是否相同
                        self.neko[y-1][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x+1] = 7#是的話，將全部轉換為肉球圖案
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        for y in range(8, -1, -1):#將格數由上至下依序檢查
            for x in range(8):#由左至右檢查
                if self.neko[y][x] > 0:
                    if self.neko[y+1][x] == 0:#若下面一格為空格
                        self.neko[y+1][x] = self.neko[y][x]#使之落下(上面格子的圖案跑到下面一格)
                        self.neko[y][x] = 0
        ...
        for y in range(8, -1, -1):
            for x in range(8):                
                if self.neko[y][x] > 0 and self.neko[y+1][x] == 0:
                    drop = False
                    print(self.neko[y][x], self.neko[y+1][x])
                    break                        
        return drop
