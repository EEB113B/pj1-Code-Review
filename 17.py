...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        for y in range(1, 9): 
            for x in range(8):
                if self.check[y][x] > 0:
                    if self.check[y+1][x+1] == self.check[y][x] and self.check[y-1][x-1] == self.check[y][x]:       #左上右下
                        self.neko[y+1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y-1][x-1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去
                    if self.check[y-1][x+1] == self.check[y][x] and self.check[y+1][x-1] == self.check[y][x]:       #右上左下
                        self.neko[y-1][x+1] = 7
                        self.neko[y][x] = 7
                        self.neko[y+1][x-1] = 7 # 此處 7 的意思是把此位置換成肉球圖片顯示，代表相連後被消去  
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        down = True
        for x in range(8):                          #判斷落下的貓咪方塊下方是否有已存在的貓咪方塊，若無則繼續往下一格落下。    
            for y in range(9): 
                if self.neko[y][x] > 0:
                    if self.neko[y+1][x] == 0:
                        ...
                        self.neko[y][x] = 0
                        break
        for X in range(8):                          #判斷落下方塊是否落到可落下的最底層
            for Y in range(1,10): 
                if self.neko[Y][X] == 0:
                    if self.neko[Y-1][X] != 0:
                         down = False
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        over = False 
        for x in range(8):
            if self.neko[0][x] > 0:         #判斷最上層是否有方塊，若有則結束遊戲
                over = True
        return over
