...
#========================================================= Question 1 =========================================================
    ...
    def check_neko(self):
        ...
        # 2.判斷左右是否連線
        for y in range(1, 10): #除了最上面那排其他都要判斷左右是否連線
            for x in range(8):
                if self.check[y][x] > 0:#如果中間的不是0就看左右是否相同
                    if self.check[y][x-1] == self.check[y][x] and self.check[y][x+1] == self.check[y][x]:
                        self.neko[y][x-1] = 7
                        self.neko[y][x] = 7
                        self.neko[y][x+1] = 7 
        # 3.判斷斜角是否連線
        for y in range(1, 9): #取中間的為基準所以不用最上面跟最下面那排
            for x in range(8):
                if self.check[y][x] > 0:
                    #斜率為正(右上升)
...
#========================================================= Question 2 =========================================================
    ...
    def drop_neko(self):
        ...
        # <<<將你的程式寫在這邊>>>
        #1.全部都掉落一格
        for y in range(8,-1,-1):#$從最下面判斷到上面
            for x in range(8):
                if(self.neko[y+1][x] == 0):#如果下面是0就直接取代
                    self.neko[y+1][x] = self.neko[y][x]
                    self.neko[y][x]=0
        #2.預設已經掉落完畢，如果有以下條件，代表尚未掉落完畢->return False否則return True
        drop_decide= True
        for y in range(8,-1,-1):
            for x in range(8):
                #如果上面那排代號>下面那排代號，下面那排代號又是0(空白)代表尚未掉落完畢->return False)
                if(self.neko[y+1][x] < self.neko[y][x]):
...
#========================================================= Question 3 =========================================================
    ...
    def over_neko(self):
        ...
        #預設還未gameover
        over_decide = False
        for x in range(8):#如果最上面兩排都不是空白(代表碰到最上面)-> Game Over
            if((self.neko[0][x]!=0)&(  self.neko[1][x]!=0)):
                over_decide = True
        if(over_decide == True):
            return True
        if(over_decide ==False):
            return False
